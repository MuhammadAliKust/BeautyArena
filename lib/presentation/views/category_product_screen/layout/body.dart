...
import '../../../../infrastructure/models/category_product.dart';
...
import '../cateogry_product_view.dart';

class CategoryProductViewBody extends StatefulWidget {
  final String categoryID;

  const CategoryProductViewBody({Key? key, required this.categoryID})
      : super(key: key);

  @override
  State<CategoryProductViewBody> createState() =>
      _CategoryProductViewBodyState();
}

class _CategoryProductViewBodyState extends State<CategoryProductViewBody> {
  ...
  void initState() {
    checkConnection().then((value) async {
      await getApiCall(true).then((value) {
        _localList = value.data!.products!.data!;
        setState(() {});
  ...
  Future<bool> getApiData({bool isRefresh = false}) async {
    ...
    try {
      await getApiCall(isRefresh).then((value) async {
        if (isRefresh == true) {
          _localList = value.data!.products!.data!;
          totalPages = value.data!.products!.meta!.lastPage!;
        } else {
          print("Called");
          value.data!.products!.data!.map((e) {
  ...
  Future<CategoryProductModel> getApiCall(bool isRefresh) async {
    ...
    CategoryProductModel model = await CategoriesServices()
  ...
  Widget build(BuildContext context) {
    ...
    return FutureProvider.value(
        value: CategoriesServices().getProductByCategoryID(context, state,
            user.getUserDetails()!.data!.token.toString(), widget.categoryID),
        initialData: CategoryProductModel(),
        builder: (context, child) {
          var model = context.watch<CategoryProductModel>();
          return model.data == null
              ...
              : Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      if (model.data!.children != null)
                        if (model.data!.children!.isNotEmpty)
                          Container(
                            width: MediaQuery.of(context).size.width,
                                  ...
                                  Container(
                                    ...
                                    child: ListView.builder(
                                        ...
                                        padding: const EdgeInsets.symmetric(
                                            horizontal: 7),
                                        itemCount: model.data!.children!.length,
                                        itemBuilder:
                                      ...
                                      itemBuilder: (BuildContext ctx, i) {
                                        return ExploreProductWidget(
                                          model: Datum(
                                            ...
                                            crossSellingProducts: _localList[i]
